...
const App = () => {
  ...
  const [activeChatUser, setActiveChatUser] = useState(null); // چیٹ سیکیورٹی سٹیٹ
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen bg-[#FDF5F5] relative overflow-x-hidden pb-28 text-right" dir="rtl">
      ...
      <div className="relative">
        ...
        <header className="relative z-10 p-6 flex items-center justify-between">
          ...
          <div className="flex gap-4">
             ...
             <Settings className="text-[#D4AF37] cursor-pointer" onClick={() => {setActiveTab('profile'); setActiveChatUser(null);}} size={22} />
          </div>
        </header>

        {/* سرچ بار صرف ڈسکور پر، لیکن جگہ محفوظ رکھی گئی ہے */}
        <div className={`relative z-10 px-6 mt-2 mb-8 transition-opacity duration-300 ${activeTab === 'discover' ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
          <div className="relative flex items-center bg-white/10 backdrop-blur-xl border border-white/20 rounded-full px-5 py-3.5 shadow-2xl focus-within:bg-white/20">
      ...
      <main className="relative z-10 -mt-4">
        ...
        {/* مکمل ایکٹیو چیٹ سسٹم */}
        {activeTab === 'messages' && (
          <div className="animate-in fade-in duration-300 px-4">
            <Chat partner={activeChatUser} onBack={() => setActiveChatUser(null)} />
          </div>
        )}

        {activeTab === 'notifications' && <Notifications onBack={() => setActiveTab('discover')} />}
        {activeTab === 'profile' && (
          <div className="animate-in slide-in-from-left duration-300 px-4">
            {!isEditing ? (
